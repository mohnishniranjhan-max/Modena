...
import { openWhatsAppDirect, WHATSAPP_NUMBER, DEFAULT_MESSAGE } from '../../utils/whatsapp';

export default function WhatsAppWidget({ isCartOpen = false, isCheckoutOpen = false }) {
  if (isCheckoutOpen) return null;

  const handleWhatsAppClick = (e) => {
    openWhatsAppDirect(WHATSAPP_NUMBER, DEFAULT_MESSAGE, e);
  };

  return (
    <div
      className={`fixed z-50 bottom-5 sm:bottom-6 transition-all duration-300 ease-out flex items-center gap-2 ${
        isCartOpen
          ? 'hidden md:flex md:right-[440px]'
          : 'right-4 sm:right-6'
      }`}
    >
      <a
        href={`whatsapp://send?phone=${WHATSAPP_NUMBER}&text=${encodeURIComponent(DEFAULT_MESSAGE)}`}
